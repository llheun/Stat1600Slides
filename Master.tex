...
\vspace{3mm}
  ...
  140, 220, 280, 1200, 380, 560, 400, 880, 700, 670
...
\item<5->
\begin{equation*}
\bar{X} = \frac{140 + 220 + \cdots + 670}{10} = 543
\end{equation*}
\end{itemize}
...
\begin{itemize}
\item<1-> That is, the average rent for 2-bedroom apartments in  Kalamazoo area is \$ 543 in our sample
...
140, 220, 280, 380, 400, 560, 670, 700, 880, 1200
\item<2->
\begin{equation*}
...
\item<4-> The MED remains unchanged (\$ 480) but $\bar{X} = 539$, down by from the original data (\$ 543).
\end{itemize}
\end{frame}
...
{\footnotesize{
\begin{itemize}
\item<1-> Step 1. Compute $\bar{x} = 543$
...
\begin{equation*}
SD = \sqrt{ \frac{\ensuremath{9.6921\times 10^{5}}}{ 10 - 1 }} = 328.2
\end{equation*}
...
\begin{itemize}
\item<1-> Recall that the mean and SD are \$ $543 \pm 328.2$ \\ ($\pm$ means `give or take')
\item<2-> Get a roommate and pay half the rent: \$ $271.5 \pm 164.1$ %$323  ± $52
\item<3-> No roommate but has contribution of \$100 per month from parents: $271.5 \pm 328.2$    % $526 ± $104
\end{itemize}
\end{frame}
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning in chisq.test(MX12A, correct = TRUE): Chi-squared approximation may be incorrect}}\end{kframe}% latex table generated in R 3.5.1 by xtable 1.8-2 package
% Thu Jul 26 14:50:44 2018
\begin{table}[ht]
\centering
\begin{tabular}{rrrrr}
 ...
 & 2.0-2.5 & 2.5-3.0 & 3.0-3.5 & 3.5-4.0 \\ 
  \hline
not returned 2nd year & 16.08 & 3.42 & 7.24 & 11.26 \\ 
  not returned 3rd year & 13.12 & 2.79 & 5.90 & 9.19 \\ 
  returned 3rd year & 50.79 & 10.79 & 22.86 & 35.56 \\ 
   \hline
\end{tabular}
\end{table}
...
{\small{
\begin{equation*}
E_i = \frac{\texttt{row total (i)} \times \texttt{col total (j)} }{\texttt{Grand Total}}
\end{equation*}
}}
\end{itemize}
\end{frame}
...
% \item<1-> The differences of Head Injury Criterion (HIC) Driver -- Passenger: \\
% DFT11b
% \item<2-> Mean difference: $\bar{D} = mn11q$
% \item<3-> Variation measures
% 
% \begin{minipage}[ht]{3.3cm}
% 
...
% \item<4-> A 95\% CI for mean difference in HICs (Driver - Passenger):
% \begin{equation*}
%   (mn11q - me11q, mn11q + me11q) \Rightarrow (lbnd11p, ubnd11p)
% \end{equation*}
% 
%   which \textbf{excludes} zero (0) and hence the mean difference in HICs is  statistically \textbf{significant}.
% 
